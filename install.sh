...
echo -e "  ${CW}Personal Resume Website${NC}  ${DIM}+${NC}  ${CG}One-Click Installer${NC}"
echo ""
sleep 0.05
echo -e "  ${DIM}┌─────────────────────────────────────────────────┐${NC}"
...
if [[ -f "${SCRIPT_DIR}/Backend/ResumeAPI/ResumeAPI.csproj" ]]; then
    ...
    git clone --depth=1 https://github.com/YOUR_USERNAME/YOUR_REPO.git "$INSTALL_DIR" \
        || error "Could not clone repository. Update the GitHub URL in install.sh"
fi
